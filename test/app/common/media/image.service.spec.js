System.register(['../../../../app/common/media/image.service', 'angular2/http/testing', 'angular2/core', 'angular2/http'], function(exports_1) {
    var image_service_1, testing_1, core_1, http_1;
    return {
        setters:[
            function (image_service_1_1) {
                image_service_1 = image_service_1_1;
            },
            function (testing_1_1) {
                testing_1 = testing_1_1;
            },
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (http_1_1) {
                http_1 = http_1_1;
            }],
        execute: function() {
            describe('ImageUploader', function () {
                var backend, http, imageService;
                beforeEach(function (done) {
                    var injector = core_1.Injector.resolveAndCreate([
                        testing_1.MockBackend,
                        core_1.provide(http_1.Http, {
                            useFactory: function (backend) { return new http_1.Http(backend, new http_1.BaseRequestOptions()); },
                            deps: [testing_1.MockBackend]
                        }),
                        image_service_1.ImageService
                    ]);
                    http = injector.get(http_1.Http);
                    backend = injector.get(testing_1.MockBackend);
                    imageService = injector.get(image_service_1.ImageService);
                    done();
                });
                it('should return image link on upload', function (done) {
                    var link = 'http://link.png';
                    backend.connections.subscribe(function (connection) {
                        connection.mockRespond(new http_1.Response(new http_1.BaseResponseOptions().merge({
                            body: { link: link }
                        })));
                    });
                    imageService
                        .upload('data:image/png;base64,')
                        .subscribe(function (res) {
                        expect(res.link).toBe(link);
                        done();
                    });
                });
                it('should delete image', function (done) {
                    backend.connections.subscribe(function (connection) {
                        connection.mockRespond(new http_1.Response(new http_1.BaseResponseOptions().merge({
                            body: {}
                        })));
                    });
                    imageService
                        .delete()
                        .subscribe(function (res) {
                        expect(res.link).toBe(link);
                        done();
                    });
                });
            });
        }
    }
});
//# sourceMappingURL=image.service.spec.js.map